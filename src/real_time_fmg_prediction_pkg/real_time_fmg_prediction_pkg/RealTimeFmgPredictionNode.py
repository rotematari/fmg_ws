...
class RealTimeSystem(Node):
    ...
    def load_config(self,):
        ...
        pprint.pprint(self.RealTimeconfig)
        ...
        #     # ])
        # Get all parameters
        # all_params = self.get_parameters_by_prefix('config')
        # self.get_logger().info(f'Loaded parameters: {config}')
        # Iterate through all parameters and store them in the config dictionary
        # for name, param in all_params.items():
        #     self.RealTimeconfig[name] = param.value
        #     self.get_logger().info(f'{name} -> {self.RealTimeconfig[name]}\n' )
    ...
    def initialize_device(self):
        ...
        self.get_logger().info(f'cuda is available {torch.cuda.is_available()}')
        self.get_logger().info(f'device {self.RealTimeconfig["device"]} ')
    ...
    def initialize_scaler(self):
        ...
        std_feature_scaler_state = self.checkpoint['feature_scaler_state']
        std_label_scaler_state = self.checkpoint['label_scaler_state']
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        shared_dir = get_package_share_directory("real_time_fmg_prediction_pkg")
        self.get_logger().info(f"shared_dir{shared_dir}")
        self.get_logger().info(f"checkpoint_path{checkpoint_path}")
    ...
    def calculate_quaternion(self, point_1, point_2):
        # Step 1: Calculate the vector from the point_1 to the point_2
        vector = np.array(point_2) - np.array(point_1)
        position = vector / 2  # Midpoint between point_1 and point_2
        ...
        # Step 4: Calculate the rotation that aligns z-axis with the vector_norm
        # Rotation.from_rotvec creates a rotation from a rotation vector
        # Rotation.align_vectors finds the quaternion that aligns two vectors
        rotation, _ = R.align_vectors([vector_norm], [z_axis])

        # Get the quaternion [x, y, z, w] using scipy's Rotation class
    ...
    def publish_pose(self, prediction):
        ...
        # self.get_logger().info(f'{prediction}')
        shoulder_pred = np.array([-0.1462,-0.320,0.5711], dtype=float)
        elbow_pred = np.array(prediction[:3], dtype=float)
        wrist_pred = np.array(prediction[3:], dtype=float)
